...
spec:
  ...
  parameters:
    ...
    # Hidden parameter (pre-configured, not prompted to users)
    - name: folderPath
      title: Folder Path
      type: string
      required: true
...
| `hidden: true` | Parameter won't be prompted, always uses default (for pre-configured infrastructure values) |
